...
def make_two_settings_matrices(
    ...
    if region != "United_States":
        region_data_file = "United_States_subnational_" + region + "_age_distribution_85.csv"
        settings_data_dict = dict([(elt.split("_")[setting_index], np.loadtxt(elt, delimiter=',', dtype=np.float64, skiprows=0)) for elt in all_settings_data_files if region in elt])
    else:
        region_data_file = "United_States_country_level_age_distribution_85.csv"
        settings_data_dict = dict([(elt.split("_")[setting_index], np.loadtxt(elt, delimiter=',', dtype=np.float64, skiprows=0)) for elt in all_settings_data_files if ("country" in elt) and ("_F_" in elt)])
    # Make sure region_data_file exists
    assert os.path.exists(path_to_population_data+"/"+region_data_file),f"The {region} file does not exist"
    # Load territory data
...
def create_age_grouped_CM(
    ...
    assert setting_CM.shape[0] == setting_CM.shape[1],"Baseline contact matrix is not square."
    ...
    assert dc.MINIMUM_AGE < dc.AGE_LIMITS[0] < dc.AGE_LIMITS[-1] < 84,"The entered age limits are not compatible"
    # Check if the upper bound of the age limits is greater than the lower bound 
    assert dc.AGE_LIMITS[0] < dc.AGE_LIMITS[1] + 1,"The bounds for the age limits are not proper"
    ...
    for i, grp_out in enumerate(age_groups):
        for j, grp_in in enumerate(age_groups):
            ...
            pop_prop_slice = np.reshape(
                pop_prop_slice.to_numpy(), (-1,1)
            )
...
def load_demographic_data() -> dict[str, dict[str, list[np.ndarray, np.float64, list[float]]]]:
    """
    ...
    assert os.path.exists(dc.MIXING_PATH),f"The base path {dc.MIXING_PATH} does not exist as it should"
    assert os.path.exists(path_to_settings_data),f"The path to the contact matrices does not exist as it should"
    assert os.path.exists(path_to_population_data),f"The path to population-rescaled age distributions does not exist as it should"
    ...
    for r in cf.REGIONS:
        ...
        try: 
            ...
            print(f"Something went wrong with {region} and produced the following error:\n\t{e}")
...
def plot_ode_solution(sol, plot_compartments=["s", "i", "r"], sum_across_axis = 2, save_path = None):
    sol = np.array(sol)
    get_indexes = [mc.idx.__getitem__(compartment.strip().upper()) for compartment in plot_compartments]
    if sum_across_axis:
