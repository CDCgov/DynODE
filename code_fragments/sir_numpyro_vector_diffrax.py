...
population = (
    10000 * n_age_groups * target_population_fractions
)  # np.array([.25, .75])
...
def model(times, incidence):
    # Parameters
    initial_infections = numpyro.sample(
        "initial_infections", dist.Exponential(1.0)
    )
    ...
    infectious_period = numpyro.sample(
        "infectious_period", dist.Exponential(1.0)
    )
    ...
    solution = diffeqsolve(
        ...
        dt0,
        initial_state,
        args=(beta, gamma, contact_matrix),
        saveat=saveat,
    )
...
mcmc.run(
    PRNGKey(8675309), times=np.linspace(0.0, 100.0, 101), incidence=incidence
)
mcmc.print_summary()
