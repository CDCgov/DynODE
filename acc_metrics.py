...
def mcmc_accuracy_measures(state, particles_per_chain):
    ...
    for chain in range(nchain):
        ...
        for particle_index in particle_indexes:
            ...
            pred_hosps_chain.append(
                jnp.array(hosps)[jnp.array(obs_hosps_days), ...]
            )
    ...
    obs_hosps = jnp.tile(
        jnp.array(obs_hosps), (nchain, len(particle_indexes), 1, 1)
    )
    ...
    posteriors_selected = {
        key: np.array(value)[:, particle_indexes]
...
mcmc_accuracy_measures(state="AK", particles_per_chain=50)
