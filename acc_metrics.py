...
def compare_elpd_per_state(suffix1, suffix2):
    # Does not need to invoke the mcmc_accuracy_measures function!
    # It'll access two CSV files generated by the mcmc_accuracy_measures calculated in two distinct models, and identified by the suffixes.
    # grab for a fixed stated the elpd number and the SE.
    # Calculate the difference in ELPD:
    # ΔELPD = ELPD_A - ELPD_B
    # Calculate the standard error of this difference:
    # SE(ΔELPD) = sqrt(SE_A^2 + SE_B^2)
    # Where SE_A and SE_B are the standard errors of ELPD_A and ELPD_B respectively.
    # Calculate the z-score:
    # z = ΔELPD / SE(ΔELPD)
    # Significance criterion:
    # If z > 2 or z<-2, the difference is considered significant.
    return
...
if __name__ == "__main__":
    ...
    az_output_path = "/output/fifty_state_6strain_2204_2407/smh_6str_prelim_6/"
    suffix = "v0"
    dframe = {}
    for st in states:
        try:
            ...
            result = mcmc_accuracy_measures(
                state=st, particles_per_chain=200, initial_model_day=560, variant=True
            )
