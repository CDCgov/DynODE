...
def get_state_likelihood(state, fitting_path):
    model_day = 800
    ...
    checkpoint_file = os.path.join(state_config_path, "checkpoint.json")
    with open(checkpoint_file) as cpf:
        posteriors = json.load(cpf)

    del posteriors["final_timestep_s"]
    del posteriors["final_timestep_e"]
    del posteriors["final_timestep_i"]
    del posteriors["final_timestep_c"]
    ...
    obs_var_sd = 120 / jnp.sqrt(jnp.sum(inferer.config.POPULATION))

    ll = numpyro.infer.util.log_likelihood(
        inferer.likelihood,
        posterior_samples=processed_posteriors,
        obs_hosps=obs_hosps,
    ...
    return ll, processed_posteriors
...
if __name__ == "__main__":
    ...
    path = "/output/fifty_state_5strain_2202_2404/SMH_5strains_240807_v16/"
    ll, posterior = get_state_likelihood(state, path)
    print({key: values.shape for key, values in ll.items()})
    ll_incd = az.from_dict(
        posterior=posterior,
        log_likelihood={"log_likelihood": np.sum(ll["incidence"], axis=3)},
    )
    ll_incd_loo = az.loo(ll_incd)
    ll_varprop = az.from_dict(
        posterior=posterior,
        log_likelihood={
            "log_likelihood": np.sum(ll["variant_proportion"], axis=3)
        },
    )
    ll_varprop_loo = az.loo(ll_varprop)
    print(ll_incd_loo)
    print(ll_varprop_loo)
    # ll_varprop = az.from_dict(log_likelihood=ll["variant_proportion"])
    # print(az.loo(ll_varprop))
    # sum_ll = {key: np.sum(values, axis=(1, 2)) for key, values in ll.items()}
    # print(sum_ll)
