...
class SMHInferer(MechanisticInferer):
    ...
    def likelihood(
        ...
        if infer_mode:
            ...
            # for x[0:7, :] -> y[0, :]
            # y.shape = [65, 4]
            obs_hosps_interval = jnp.array(obs_hosps_interval)
            vmr = jnp.var(obs_hosps_interval, axis=0) / jnp.mean(
                obs_hosps_interval, axis=0
            )
            # min_vmr = jnp.min(vmr)
            # value = jnp.greater(
            #     min_vmr, 1.1
            # )  # This returns a boolean value# Define the functions to perform based on the condition

            # def true_fn(vmr):
            # vmr = jnp.array(vmr)
            mu = jnp.mean(obs_hosps_interval, axis=0)
            pos_vmr = jnp.maximum(
                vmr - jnp.array([1] * obs_hosps_interval.shape[1]),
                jnp.array([0.01] * obs_hosps_interval.shape[1]),
            )
            alpha = mu / pos_vmr
            mult = numpyro.sample(
            ...
            # Use jax.lax.cond to handle the condition
            # lax.cond(value, true_fn, false_fn, vmr)
            ## Seroprevalence
