...
class ProjectionParameters(MechanisticInferer):
    ...
    def _solve_runner(
        ...
        """an Overridden version of AbstractParameters._solve_runner() designed
        specifically to rework the initial state to fit the new compartment
        definitions before solving the transmission model.

        The fitting period uses 5 strains, while projection uses 7, so we must expand
        those dimensions and reset the C compartment.
    ...
    def generate_downstream_parameters(self, parameters: dict) -> dict:
        """
        OVERRIDEN FUNCTION to add INITIAL_STATE into the parameters list
        as we are loading posterior particles from a previous fit when projecting.
        This function will only be called in the context of a loaded posterior particle
        within `self.load_posterior_particle()`
        ...
        try:
            ...
            waning_rates = np.array(
                [
                    ...
                    for waning_time in parameters["WANING_TIMES"]
                ]
            )
            ...
            # for more information on why this works look into MechanisticInferer.load_posterior_particle()
            ...
            parameters["POPULATION"] = self.retrieve_population_counts(
            ...
            if (
                self.config.SAMPLE_STRAIN_X_INTRO_TIME
                and parameters["INTRODUCTION_TIMES"][1]
            ):
    ...
    def get_parameters(self):
        ...
        While this appears to be the exact same as super().get_parameters()
        the function it calls is overriden, and thus does very different things.

        it specifically difers because it does not use `self.POPULATION` since
    ...
    def likelihood(
        ...
        # filtering predicted hospitalizations into observed days, then updating sampling algo
        ...
        # filtering predicted sero prev into observed days, then updating sampling algo
        sim_seroprevalence = dct["sim_seroprevalence"]
        # filter to just observed days
        ...
        # filtering predicted var proportions into observed days, then updating sampling algo
