...
from exp.fifty_state_2202_2307_3strain.inferer_smh import SMHInferer
from exp.fifty_state_2202_2307_3strain.single_state_runner_sero import (
...
model_day = 416
suffix = "_v16a_ny"
az_output_path = "/output/fifty_state_2202_2307_3strain/smh_epoch_1_240513_3"
...
def retrieve_inferer_obs(state):
    ...
    var_data = var_data[var_data["strain"] < 3]
...
def retrieve_post_samp(state):
    ...
    post_samp.pop("april_1st_timestep_s", None)
    post_samp.pop("april_1st_timestep_e", None)
    post_samp.pop("april_1st_timestep_i", None)
    post_samp.pop("april_1st_timestep_c", None)
    fitted_medians = {
...
def plot_obsvfit(
    ...
    for i, sh in enumerate(sim_hosps_list):
        if i == 0:
            axs[0].plot(
                dates[obs_hosps_days],
                sh,
                label=["0-17", "18-49", "50-64", "65+"],
                alpha=0.1,
            )
        else:
            axs[0].plot(dates[obs_hosps_days], sh, alpha=0.1)
