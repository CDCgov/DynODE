...
from run_task import rework_initial_state
...
def retrieve_inferer_obs(state, az_output_path, model_day):
...
def retrieve_fitted_medians(state, az_output_path):
...
def retrieve_timeline(state, az_output_path):
...
def process_plot_state(state, az_output_path):
    timeline = retrieve_timeline(state, az_output_path)
    fitted_medians = retrieve_fitted_medians(state, az_output_path)
    ...
    (
        ...
        obs_var_prop,
        obs_var_days,
    ) = retrieve_inferer_obs(state, az_output_path)
...
def save_states_pdf(output_path, job_id):
    ...
    pool = mp.Pool(5)
    figs, median_dfs = zip(
        *pool.map(process_plot_state, [(st, job_path) for st in states])
    )

    pdf_pages = PdfPages(
        os.path.join(job_path, "obs_vs_fitted_%s.pdf" % job_id)
    )
    for f in figs:
        pdf_pages.savefig(f)
        plt.close(f)
...
if __name__ == "__main__":
    ...
    save_states_pdf(output_path, job_id)
