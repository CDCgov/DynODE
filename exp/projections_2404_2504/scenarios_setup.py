...
CONFIG_MOLDS = [
    "exp/projections_2404_2504/template_configs/config_global.json",
    "exp/projections_2404_2504/template_configs/scenario_template.json",
]

EXP_FOLDER = "exp/projections_2404_2504"
SCEN_CSV = "exp/projections_2404_2504/scenarios.csv"
...
def create_multiple_scenarios_configs(state_config, state_abb, subdir_path):
    ...
    for index, row in df.iterrows():
        ...
        kpit = row["kp_intro_time"]
        mult = row["r0_percentage"]
        ...
        new_config_file_path = os.path.join(
            subdir_path,
            f"vs{str(vs)}_it{it}_ie{str(ie)}_ve{str(ve)}_mult{str(mult)}_kpit{str(kpit)}.json",
        )
